...
const MAP: Record<string, AgentPresentation> = {
  ...
  dog: {
    ...
    accent: '#0f766e',
    tagline: 'Contracts',
    why: "Core trait: loyalty & persistence. A dog guards what's ours and doesn't let go — it reads every agreement a client sends us against what we actually quoted, and never lets an exposure slip through because the contract was long and the deadline was short.",
    hasDashboard: true,
    howItWorks: {
      title: 'איך כלב עובד',
      rtl: true,
      steps: [
        { icon: 'FolderPlus', label: 'בוחרים פרויקט', sub: 'תיקייה בדרייב', who: 'you' },
        { icon: 'Search', label: 'מאתר את הקבצים', sub: 'הסכם + הצעת מחיר', who: 'agent' },
        { icon: 'ListChecks', label: 'בודק מול הצ׳קליסט', sub: '7 נושאים מוגדרים', who: 'agent' },
        { icon: 'PenLine', label: 'כותב ממצאים', sub: 'בעיה + נוסח מתוקן', who: 'agent' },
        { icon: 'Hand', label: 'אתם עורכים ושולחים', sub: 'הטבלה היא המכתב', who: 'you' },
      ],
    },
    about: {
      intro:
        'כלב הוא סוכן החוזים — משווה הסכם שהתקבל מלקוח מול הצעת המחיר ששלחנו, ומחזיר רשימת ממצאים משפטיים מוכנה לעריכה ולשליחה.',
      why:
        'הכלב נאמן ועקבי — שומר על מה ששלנו ולא מרפה. בדיוק מה שנדרש כדי לעבור על כל הסכם ולוודא שלא נכנסנו לחשיפה שלא הסכמנו לה.',
      autoTitle: 'מה כלב עושה',
      autoItems: [
        'מאתר לבד את ההסכם בתיקיית "חוזה" ואת הצעת המחיר בתיקיית "הצעות מחיר" של הפרויקט — ומראה לכם מה בחר לפני שהוא רץ.',
        'קורא את שני המסמכים (PDF במקור, Word ו-Google Docs כטקסט) ומשווה ביניהם.',
        'בודק אך ורק את הנושאים שבצ׳קליסט — ומתעלם מסעיפים גנריים שמופיעים בכל חוזה ייעוץ.',
        'מחזיר לכל ממצא: עמוד, סעיף, תיאור הבעיה עם ציטוט, ונוסח מתוקן מוצע.',
        'לפי בחירתכם — משווה גם להסכמים שכבר חתמנו עם אותו לקוח, ומוסיף לכל ממצא הערה כיצד עמד אותו סעיף שם ("הופיע וטופל", "לא הופיע", "נוסח מתון יותר").',
      ],
      userTitle: 'מה אתם עושים',
      userItems: [
        'בוחרים את תיקיית הפרויקט ומאשרים את הקבצים שהוא זיהה.',
        'עורכים את הממצאים בטבלה — מנסחים מחדש, מוחקים מה שלא רלוונטי.',
        'מעתיקים את המכתב ושולחים ללקוח.',
        'מתחזקים את הצ׳קליסט: מוסיפים נושא שצריך להיבדק, או מורידים נושא שכבר לא מעניין.',
      ],
      chatTitle: 'מה עוד מתוכנן',
      chatItems: [
        'צ׳אט לכל בדיקה — "למה זה בעיה?", "תרכך את התיקון בסעיף 6.2".',
        'למידה מהעריכות שלכם, כדי שהניסוח בבדיקה הבאה יהיה קרוב יותר לשלכם.',
        'מעקב אחרי חוזים וגבייה — החצי השני של הכלב.',
      ],
    },
  },
}
