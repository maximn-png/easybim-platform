...
// Durable, per-agent guidance lines over the shared AgentGuidance model.
// Agent-agnostic home for what peacock/guidance.ts and squirrel/guidance.ts each
// keep a local copy of; new agents should import from here.

export async function getGuidance(agentKey: string): Promise<string[]> {
  await connectDB()
  const docs = await AgentGuidance.find({ agentKey, active: true })
    .sort({ createdAt: -1 })
    .limit(50)
    .lean()
