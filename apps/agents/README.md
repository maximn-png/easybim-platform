...
- `PeacockAnalytics.tsx` — the chart is columns, one series, **no legend** (the title names it) and **no dual axis**: engagements live in the tooltip and the summary line, never a second y-scale. `#1e248c` (the platform navy — Peacock was restyled to the EPM design language 2026-09-06) was checked with the dataviz palette validator (lightness band / chroma floor / ≥3:1 contrast all pass on white). Values are never tooltip-gated — there is a table-view twin, and the peak + latest columns are directly labelled.
...
**Squirrel dashboard (Peacock-style landing + two cards).** `presentation.squirrel.hasDashboard` makes `SquirrelDashboard.tsx` the landing; the chat is docked beside it (`PeacockChatDock` now takes an `agentName`) and the full-page `ChatShell` is one click away. Both cards are also chat tools (`lib/agents/squirrel/insights.ts`: `completed_projects_analytics`, `client_analytics`), so a card and an answer never disagree.
- **Completed Projects** (`lib/agents/squirrel/completed.ts` → `/api/dashboard/squirrel/completed`, UI `CompletedProjects.tsx`). Joins three sources on the project number: **MA-004 read live** (`projectsBoard.ts`, 5-minute memo, `?refresh=1` to bypass) for who is `DONE`, their `סוג פרויקט` (`dropdown8`) and the per-department ₪; EPM `projects` for the number→`_id` bridge; EPM `timeentries` for the hours actually spent. Verdict = the office's own **כדאיות** rule (MA-004 `formula1`): under `שכט סופי ÷ 300` = `++`, past `÷ 225` = `−`, between = `+`. **Three departments** (EPM's hours page has two): `modelMgmt` = `formula_mkng494f`, `superposition` = `formula_mkngmc97` (תאום מערכות only), `modelling` = `numeric_mkxsce4b` + `numeric_mkxsjd0j`; each ÷300 is that department's hour bank. Subject→department is one **office-wide** map (`SquirrelSetting`, key `subjectDepartment`, editable on the card) so all completed projects are judged by one rule — deliberately separate from EPM's per-project `hoursConfig`.
- ⚠️ **Counting gotcha (verified 2026-09-06):** MA-004 has **36** items at `DONE`, EPM has **26** projects at `Done`, and only **24** carry both a budget and a timesheet. A project with no timesheet reads as zero hours, which against any bank would score as our most profitable job ever — so `judge()` takes a `measured` flag and returns `unknown` instead, and every group ratio is computed over measured projects only. For the same reason department rollups only count projects that carry a price for that department (most non-C projects fill `שכט סופי` but leave the discipline columns blank); hours worked there anyway are reported as `spentUnpriced` rather than dropped. MA-004's own `כדאיות` column disagrees with ours on 8 of 36 because it runs off the hand-entered `סהכ שעות` (`numbers8`), not the timesheet — the card flags the disagreement rather than hiding it.
- **Analytics by Client** (`lib/agents/squirrel/clients.ts` → `/api/dashboard/squirrel/clients`, UI `ClientAnalytics.tsx`). Pure `QuoteRecord` aggregation, groupable by any of the six parties. Won = `Approved`/`Done`, lost = `Declined`, open = `Pending`/`Working on it`/`More Info is needed`. Per party: win rate, ₪ won vs lost, first/last quote, median response days, decline streak, and flags — `losingStreak` (3+ consecutive declines), `coldContact` (no quote sent in 5+ months), `allDeclined`, `strong`. The **Worth a call** list is those rows sorted by severity. Quotes with no value in the chosen party column are counted and reported (`unassigned`) rather than bucketed into "(none)" — 106 of 471 have no `יזם ראשי`, and `מזמין העבודה` often covers them.
- ⚠️ **Chart palettes** live in `app/dashboard/[agentKey]/squirrelMeta.ts` and are validated (dataviz six checks) on the card surface: departments `#4f5bd5 / #44b8d3 / #f59e0b` (categorical, fixed order), verdicts and quote outcomes are a **status** palette always shipped with a badge or label. The lighter steps sit under 3:1 contrast, which is only legal because every mark carries a visible number — don't strip the labels.
...
2. **Delete the stale `easybim-agents` Vercel project** (`easybim-agents.vercel.app`). It is a second deployment of `apps/agents` built from an old branch (`dev1`/`dev2`/`feature/Polina` still carry `peacock/board.ts`) with dev env vars — its sign-in redirect still points at `http://localhost:3000`, while production `agents.easybim.co.il` points at `https://app.easybim.co.il`. It runs the same weekly `"0 6 * * 0"` cron and kept creating 2 items on the Monday board every Sunday (~06:32 UTC, ~30 min after the real run) using the personal `MONDAY_API_TOKEN`. Delete that project, then the stale branches. (`MONDAY_API_TOKEN` stays — Squirrel and EPM use it, and the import script needs it if re-run.)
