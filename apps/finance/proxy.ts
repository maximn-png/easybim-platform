...
// the user must hold the `finance` card grant (Clerk publicMetadata.apps) —
// finance data is restricted to the finance team.
...
export default clerkMiddleware(async (auth, req) => {
  ...
  if (!canAccessApp(access, 'finance')) {
    return NextResponse.redirect(new URL(`${portalUrl}/no-access?app=finance`))
  }
})
