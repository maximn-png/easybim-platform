...
import {
  UNASSIGNED, DISCIPLINE_LABELS, FIXED_FILTER_KEYS,
  groupLabelHe, localizeGroup, monthLabelHe, issueParamValue,
} from './reportLocale'

// The Export tab of the Reports drawer — previously ExportReportModal. The flow
// is unchanged (template → filters → recipients → Gmail draft / internal save);
// only the surrounding chrome moved to the drawer.
...
export default function ExportReportPanel({
  ...
  // The rasterization nodes live in a portal so they keep their layout even when
  // the drawer hides this tab (a display:none ancestor would zero their size).
  ...
  // On mount: seed the body/variant, inherit the page's selections, and fetch
  // the ACC team once for the recipient suggestions.
  //
  // Note the filters are NOT seeded from the template here — the page's own
  // selections win on open (the template's issueType/discipline hints only kick
  // in when the user switches template, via pickTemplate). Draft is never
  // reportable, so it's stripped from the inherited statuses.
  useEffect(() => {
  ...
  // Rasterize a hidden chart node to base64 PNG (fonts settled first).
  const chartToBase64 = async (node: HTMLDivElement | null): Promise<string | undefined> => {
  ...
  const screenshotToBase64 = async (): Promise<string | undefined> => {
    ...
    try {
      ...
      return undefined // screenshot optional
    }
  }
  ...
  const handleSaveInternal = async () => {
    ...
    try {
      ...
      const chartPngBase64 = await chartToBase64(docChartRef.current)
      const screenshotPngBase64 = await screenshotToBase64()
      const previewHtml = buildEmailHtml({
        bodyText, links, highlightPhrases: resolved.highlightPhrases,
        hasChart: true, hasScreenshot, hasLegend: includeLegend,
  ...
  return (
    <div dir="rtl">
      ...
      {/* ── Hidden snapshot nodes (off-screen, for PNG rasterization) ──
          Rendered through a portal to document.body: the drawer hides inactive
          tabs with display:none, which would otherwise collapse these to 0×0 and
          produce a blank chart image. They also sit outside the RTL container,
          so they set dir="rtl" themselves. */}
