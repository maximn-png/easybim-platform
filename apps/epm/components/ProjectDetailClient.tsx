...
import { StatCard, DisciplineBar, CANONICAL_DEFAULT, MILESTONE_DISCIPLINE_COLOR } from './StatCards'
            ...
            {mondayUpdates !== null && mondayUpdates.length > 0 && updatesSummary !== '' && (
              <div className="shrink-0 rounded-xl border border-[#44b8d3]/30 bg-gradient-to-br from-[#f0f7ff] to-[#f6fbfd] px-3 py-2.5">
                ...
                {!summaryCollapsed && (updatesSummary === null ? (
                  ...
                  // The summary arrives as heading lines (כללי / תאום מערכות /
                  // ניהול מודל BIM) with "• " bullet lines under each.
                  <div dir="auto" className="text-[11.5px] leading-relaxed text-gray-700 mt-1">
                    {updatesSummary.split('\n').map((line, i) => {
                      const t = line.trim()
                      if (!t) return null
                      const bullet = /^[•\-*]\s*/.test(t)
                      return bullet ? (
                        <p key={i} className="flex gap-1.5 ps-1">
                          <span className="text-[#44b8d3] shrink-0">•</span>
                          <span>{t.replace(/^[•\-*]\s*/, '')}</span>
                        </p>
                      ) : (
                        <p key={i} className="font-bold text-[#1e248c] mt-1.5 first:mt-0">
                          {t.replace(/:$/, '')}
                        </p>
                      )
                    })}
