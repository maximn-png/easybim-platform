...
import { getApsToken } from '@/lib/services/apsService'
...
export async function GET() {
  const [clerk, mongo, monday, aps] = await Promise.all([
    check(async () => {
    ...
    check(async () => {
      await getApsToken()
      return '2-legged token issued'
    }),
  ])

  const checks = { clerk, mongo, monday, aps }
  const ok = Object.values(checks).every((c) => c.ok)

  return NextResponse.json({ ok, app: 'epm', checks }, { status: ok ? 200 : 503 })
}
