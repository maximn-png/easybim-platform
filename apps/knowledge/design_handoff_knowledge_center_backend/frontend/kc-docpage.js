...
.dp-tb,.dp-web{font-family:var(--dp-fb);line-height:1.62;color:var(--dp-ink0);direction:ltr}
...
.dp-toc-i{display:flex;align-items:center;gap:9px;padding:5px 9px;border-radius:8px;text-decoration:none;color:var(--dp-ink0);transition:background .12s var(--dp-ease);text-align:left}
.dp-toc-i:hover{background:#f2f5ff}
.dp-toc-n{width:18px;height:18px;flex-shrink:0;border-radius:5px;background:#e7eefe;color:var(--dp-navy);font-family:var(--dp-fm);font-size:9.5px;font-weight:600;display:grid;place-items:center}
.dp-toc-t{flex:1;min-width:0;display:flex;flex-direction:column;line-height:1.25;text-align:left}
.dp-toc-en{font-size:11px;font-weight:500;color:var(--dp-ink2);order:2}
.dp-toc-he{font-size:12.5px;font-weight:700;color:var(--dp-ink0);text-align:left;order:1}
...
`;
  ...
  DP.tocHTML = function (d) {
    ...
    const items = d.toc.map((it, i) => {
      ...
      return '<li><a class="dp-toc-i" href="#' + esc(it.anchor) + '" onclick="return KC.DocPage.goToSection(\'' + esc(it.anchor) + '\')">' +
      '<span class="dp-toc-n">' + (i + 1) + '</span>' +
  ...
  DP.blocksHTML = function (d, opts) {
    ...
    DP.check(d).blocks.forEach(b => {
      if (b.t === 'h') {
        ...
        out += '<p dir="auto" class="dp-p' + (b.sub ? ' dp-sub' : '') + '">' + inner + '</p>';
      } else if (b.t === 'ul' || b.t === 'ol') {
        const tag = b.t;
        out += '<' + tag + ' class="dp-list dp-' + tag + (b.sub ? ' dp-sub' : '') + (b.sq ? ' dp-sq' : '') + '">' +
          b.items.map(it => '<li dir="auto">' + tech(it) + '</li>').join('') + '</' + tag + '>';
      } else if (b.t === 'callout') {
        out += '<div class="dp-callout"><i data-lucide="check-circle-2"></i><span dir="auto">' + tech(b.txt) + '</span></div>';
      } else if (b.t === 'fig') {
  ...
  DP.figureHTML = function (b, opts) {
    ...
    if (opts.static) {
      ...
      inner = src
        ...
        : '<div class="dp-fig-empty"><i data-lucide="image"></i><span dir="auto">' + esc(b.cap) + '</span></div>';
    } else {
    ...
    return '<figure class="dp-fig" data-fig="' + esc(b.id) + '">' +
      ...
      '<figcaption class="dp-cap" dir="auto">' + tech(b.cap) + '</figcaption>' +
      '</figure>';
  };
  ...
  DP.linksHTML = function (d) {
    ...
    const items = d.links.map(l =>
      ...
      '<span class="dp-related-t" dir="auto">' + tech(l.label) + '</span>' +
  ...
  DP.editableHTML = function () {
    ...
    const versionsTbl = versions.length
      ? '<div style="font-weight:700;color:#1e248c;font-size:12pt;margin:26px 0 8px">Versions</div>' +
        ...
        '</table>' : '';
    return '<!DOCTYPE html><html lang="en" dir="ltr"><head><meta charset="UTF-8"><title>' + esc(d.title) + '</title></head>' +
